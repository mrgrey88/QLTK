        ...
        void LoadInfoSearch()
        {
            using (WaitDialogForm fWait = new WaitDialogForm("Vui lòng chờ trong giây lát...", "Đang load dữ liệu..."))
            {
                try
                {
                    ...
                    //string sql = "select * from vProjectProblemAll with(nolock) where Status = 0";
                    //if (chkIsDeleted.Checked)
                    //{
                    //    sql = "select * from vProjectProblemAll with(nolock)";
                    //}

                    int year = TextUtils.ToInt(cboYear.SelectedItem);

                    string sql = "exec spProjectProblemAll " + TextUtils.ToInt(chkIsDeleted.Checked) + ",'" + ProjectCode + "', '" + ModuleCode + "', " + year;                    

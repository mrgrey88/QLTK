    ...
    public partial class frmListModuleOfProjectTPAD : _Forms
    {
                ...
                for (int i = 0; i < drs.Length; i++)
                {
                    ...
                    if (array != null)
                    {
                        for (int i1 = 0; i1 < array.Length; i1++)
                        {
                            ...
                            DataRow[] drsLink = dtLink.Select("Code = '" + partsCode + "' or Code like '" + partsCode + "-%'");
                    ...
                    for (int j = 0; j < dtLink.Rows.Count; j++)
                    {
                        //try                        
                        //{
                        string partsCode = TextUtils.ToString(dtLink.Rows[j]["Code"]);
                        decimal qty = parentQty * TextUtils.ToDecimal(dtLink.Rows[j]["QtyReal"]);
                        string material = TextUtils.ToString(dtLink.Rows[j]["VatLieu"]);
                        ...
                        //{
                            ...
                            detail.ProjectModuleId = projectModuleId;
                            detail.ProjectDirectionID = direction.ID;
                            detail.Material = TextUtils.ToString(dtLink.Rows[j]["VatLieu"]);
                            detail.ModuleCode = moduleCode;
                            detail.PartsCode = partsCode;
                            detail.PartsName = TextUtils.ToString(dtLink.Rows[j]["Name"]);
                            detail.ProjectCode = projectCode;
                            detail.Qty = qty;

                            detail.STT = TextUtils.ToString(dtLink.Rows[j]["STT"]);
                            detail.ThongSo = "TPA";
                            detail.Unit = TextUtils.ToString(dtLink.Rows[j]["UnitPart"]);
                            detail.MaVatLieu = TextUtils.ToString(dtLink.Rows[j]["MaVatLieu"]);
                            detail.UserId = "";
        ...
        private void btnCreateChiThiCN_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < drs.Length; i++)
            {
                ...
                #region Lắp ráp
                DataRow[] drsLink1 = dtLink.Select("ThongSo = 'TPA' and Unit = 'BỘ'");
                for (int j = 0; j < drsLink1.Length; j++)
                {
                    string partsCode = TextUtils.ToString(drsLink1[j]["Code"]);
                    decimal qty = parentQty * TextUtils.ToDecimal(drsLink1[j]["QtyReal"]);
                    string material = TextUtils.ToString(drsLink1[j]["VatLieu"]);
                    ...
                    //{
                        ...
                        directionDetail.ProjectModuleId = projectModuleId;
                        directionDetail.ProjectDirectionID = direction.ID;
                        directionDetail.Material = TextUtils.ToString(drsLink1[j]["VatLieu"]);
                        directionDetail.ModuleCode = moduleCode;
                        directionDetail.PartsCode = partsCode;
                        directionDetail.PartsName = TextUtils.ToString(drsLink1[j]["Name"]);
                        directionDetail.ProjectCode = projectCode;
                        directionDetail.Qty = qty;                       

                        directionDetail.STT = TextUtils.ToString(drsLink1[j]["STT"]);
                        directionDetail.ThongSo = "TPA";
                        directionDetail.Unit = "BỘ";
                        directionDetail.UserId = "";
